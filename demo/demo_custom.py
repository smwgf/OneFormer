...
if __name__ == "__main__":
    ...
    if args.input:
        iter_object = []
        for arg in tqdm.tqdm(args.input):
            current_path = Path(arg)
            if current_path.is_dir():
                extentions = ['jpg','png']                
                for extention in extentions:
                    iter_object += list(Path(current_path).glob(f'*.{extention}'))
            else:
                iter_object.append(current_path)

        for path in tqdm.tqdm(iter_object, disable=not args.output):
            ...
            if args.output:
                if len(iter_object) == 1:
                    ...
                    for k in visualized_output.keys():
                        ...
                        out_filename = os.path.join(opath, path.name)
                        visualized_output[k].save(out_filename)    
            else:
